// components/subcategory/ServerComponents/HeaderServer.tsx
...
interface Props {
  ...
  title: string
  /** Descripción de la subcategoría (subcategories.description en Markdown) */
...
export default function HeaderServer({
  ...
  const mdDescription = (subcategoryDescription ?? "").replace(/\\n/g, "\n")
  ...
  return (
    <div className={cn("mb-6 md:mb-8", className)}>
      ...
      {/* Descripción de subcategoría (Markdown → HTML) */}
      {mdDescription && (
        <div
          data-subcategory-desc
      ...
      <div className="mt-4 flex items-center justify-between">
        <h2 className="text-sm md:text-base text-foreground">
          Mejores {totalRestaurantsLabel} restaurantes de {subcategoryName} en {cityName}
        </h2>
        {orderControl ?? null}
      </div>
    </div>
