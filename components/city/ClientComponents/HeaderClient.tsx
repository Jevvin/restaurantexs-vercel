...
import HeaderServer from "@/components/city/ServerComponents/HeaderServer"
...
const TRUNCATE_MODE: "paragraph" | "words" = "paragraph" // ← cambia a "words" si prefieres
const WORD_LIMIT = 50
...
export default function HeaderClient({
  ...
  useEffect(() => {
    const desc = document.querySelector<HTMLElement>('[data-city-desc]')
    if (!desc) return

    // evita duplicar botón si se re-monta
    let button = desc.nextElementSibling as HTMLButtonElement | null
    const isToggleBtn = !!button && button.tagName === "BUTTON" && (button as HTMLButtonElement).dataset.descToggle !== undefined
    if (!isToggleBtn) {
    ...
    // Si por alguna razón no quedó botón, salimos
    if (!button) return

    // MODO A: solo primer párrafo (oculta hermanos > 1)
    const collapseToFirstParagraph = () => {
      const children = Array.from(desc.children) as HTMLElement[]
      children.forEach((el, i) => { el.style.display = i === 0 ? "" : "none" })
    }
    ...
    // MODO B: primeras N palabras (sustituye contenido por versión truncada)
    ...
    const collapseToWords = () => { desc.innerHTML = makeTruncatedHTML(WORD_LIMIT) }
    const expandAllWords = () => { desc.innerHTML = originalHTML }

    // Determina si vale la pena mostrar botón (dos párrafos o más, o más de WORD_LIMIT palabras)
    ...
    return () => {
      ...
      // Deja el DOM limpio cuando cambies de ciudad
  ...
  return (
    <>
      <HeaderServer
        ...
        title={title}
        subtitle={subtitle}              // viene desde page.tsx (de Supabase)
        cityName={cityName}
